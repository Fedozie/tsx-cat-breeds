...
const Cats = () => {
  ...
  useEffect(() => {
    ...
    const strToNumAvg = (arr: string[]): number => {
      let strToNumArr = arr.map((str) => {
          ...
          let newNum = parseFloat(numberedStr);
          return newNum
      })
  ...
  return (
    <div>
      ...
      <p>On an average, a cat can weigh {catObject.weight} kg and they can live up to {catObject.lifeSpan} years.</p>
    </div>
  );
};
